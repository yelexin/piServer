 ...
 * @api     public
 */
export function connectSqlite3 (connect) {
  /**
   * Connect's Store.
   */
   ...
   * @api     public
   */
  function SQLiteStore(options: any = {}) {
    ...
    if (
      this.db.indexOf(':memory:') > -1 ||
      this.db.indexOf('?mode=memory') > -1
    ) {
      dbPath = this.db;
    } else {
  ...
  SQLiteStore.prototype.touch = function (sid, session, fn) {
    if (session && session.cookie && session.cookie.expires) {
      ...
      this.db.run(
        'UPDATE ' +
          this.table +
          ' SET expired=? WHERE sid = ? AND ? <= expired',
